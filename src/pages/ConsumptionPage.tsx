...
export function ConsumptionPage() {
  ...
  return <div className="-mx-5 -my-8 min-h-[calc(100vh-5rem)] w-[calc(100%+2.5rem)] bg-[#eaf8fa] px-4 py-5 sm:-mx-8 sm:-my-12 sm:w-[calc(100%+4rem)] sm:px-8 sm:py-8"><div className="mx-auto w-full max-w-md space-y-4"><header><div className="flex items-start justify-between"><div><p className="text-[11px] font-black uppercase tracking-[0.1em] text-[#145d72]">Cooking</p><h1 className="text-lg font-black text-[#193b5a]">{recipeTitle ?? 'Kitchen update'}</h1></div><Link to="/app/recipes" aria-label="Close cooking flow" className="text-[#145d72]"><X size={22} /></Link></div><div className="mt-3 flex items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[#d9eef3]"><div className="h-full rounded-full bg-[#20c4d6]" style={{ width: `${currentIndex / cookingItems.length * 100}%` }} /></div><span className="text-[11px] font-bold text-[#193b5a]">Ingredient {currentIndex + 1} of {cookingItems.length}</span></div></header><div className="rounded-2xl border border-[#bdebf0] bg-white p-3"><div className="flex items-center gap-3"><div className="flex size-14 items-center justify-center rounded-xl bg-[#e8f7fa] text-[#4f8ca3]"><CircleCheck size={26} /></div><div><h2 className="font-black text-[#193b5a]">{currentItem.name}</h2><p className="text-sm font-semibold text-[#477d8d]">{currentItem.quantity} {currentItem.unit} available</p><p className="mt-1 inline-flex items-center gap-1 rounded-full bg-[#fff8d8] px-2 py-0.5 text-[10px] font-bold text-[#9b7800]"><Clock3 size={11} /> Expires {currentItem.expires.toLowerCase()}</p></div></div></div><h2 className="text-xl font-black leading-6 text-[#193b5a]">{choice === 'some' ? 'How much did you use?' : `Did you use all ${currentItem.quantity} ${currentItem.unit} of ${currentItem.name}, or do you have leftovers?`}</h2>{saveError && <p role="alert" className="rounded-xl bg-[#fff0ef] px-4 py-3 text-sm text-[#ad4147]">{saveError}</p>}{choice !== 'some' ? <div className="space-y-2"><button type="button" onClick={chooseAll} disabled={isSaving} className="flex w-full items-center gap-3 rounded-2xl border border-[#bdebf0] bg-[#f0fafb] p-4 text-left"><Check size={18} className="text-[#145d72]" /><span><strong className="block text-sm text-[#193b5a]">Used all</strong><span className="text-xs text-[#6f8b95]">Mark {currentItem.quantity} {currentItem.unit} as consumed</span></span></button><button type="button" onClick={chooseSome} disabled={isSaving} className="flex w-full items-center gap-3 rounded-2xl border border-[#bdebf0] bg-[#f0fafb] p-4 text-left"><CircleCheck size={18} className="text-[#145d72]" /><span><strong className="block text-sm text-[#193b5a]">Used some</strong><span className="text-xs text-[#6f8b95]">I have some left over</span></span></button><button type="button" onClick={chooseNone} disabled={isSaving} className="flex w-full items-center gap-3 rounded-2xl border border-[#bdebf0] bg-white p-4 text-left"><X size={18} className="text-[#6f8b95]" /><span><strong className="block text-sm text-[#193b5a]">Did not use it</strong><span className="text-xs text-[#6f8b95]">Leave it in inventory as is</span></span></button></div> : <div className="space-y-3"><input value={someAmount} onChange={(event) => setSomeAmount(event.target.value)} type="number" min={step} max={currentItem.quantity} step={step} className="h-12 w-full rounded-xl border border-[#b8d7de] bg-white px-4 text-[#193b5a]" /><button type="button" onClick={continueSome} disabled={isSaving} className="h-11 w-full rounded-xl bg-[#20c4d6] font-black text-[#063e4d]">Continue</button></div>}</div></div>
}

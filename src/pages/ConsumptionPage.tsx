...
import { consumeFoodItem, createLeftoverItem, recordMealPrepared } from '../firebase/services/food.service'
...
export function ConsumptionPage() {
  ...
  function moveNext(decision: Decision) {
    ...
    setChoice(null)
    setSomeAmount('')
    if (currentIndex + 1 >= cookingItems.length) {
      void finishCooking(nextDecisions)
    } else {
  ...
  function chooseAll() {
    if (currentItem) moveNext({ used: currentItem.quantity, leftover: 0 })
  }

  function chooseNone() {
    moveNext({ used: 0, leftover: 0 })
  }
  ...
  async function finishCooking(finalDecisions: Record<string, Decision>) {
    ...
    try {
      for (const item of cookingItems) {
        ...
        await consumeFoodItem(item.id, decision.used, item.quantity)
        if (decision.leftover > 0) await createLeftoverItem(item, decision.leftover)
      }
      if (recipeTitle) await recordMealPrepared(recipeTitle)
      setIsComplete(true)
    } catch (saveError) {
      setSaveError(saveError instanceof Error ? saveError.message : 'Unable to update your kitchen.')
      setIsSaving(false)
    }
  }

  if (isLoading) return <div className="w-full max-w-3xl rounded-3xl border border-[#cde6ed] bg-white p-10 text-center text-[#5e7f8b]">Loading your ingredients...</div>
  ...
  return <div className="-mx-5 -my-8 min-h-[calc(100vh-5rem)] w-[calc(100%+2.5rem)] bg-[#eefafd] px-4 py-5 sm:-mx-8 sm:-my-12 sm:w-[calc(100%+4rem)] sm:px-8 sm:py-8"><div className="mx-auto w-full max-w-md space-y-4"><header><div className="flex items-start justify-between"><div><p className="text-[11px] font-black uppercase tracking-[0.1em] text-[#145d72]">Cooking</p><h1 className="text-lg font-black text-[#173d4e]">{recipeTitle ?? 'Kitchen update'}</h1></div><Link to="/app/recipes" aria-label="Close cooking flow" className="text-[#145d72]"><X size={22} /></Link></div><div className="mt-3 flex items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[#d9eef3]"><div className="h-full rounded-full bg-[#20bed0]" style={{ width: `${currentIndex / cookingItems.length * 100}%` }} /></div><span className="text-[11px] font-bold text-[#173d4e]">Ingredient {currentIndex + 1} of {cookingItems.length}</span></div></header><div className="rounded-2xl border border-[#cde6ed] bg-white p-3"><div className="flex items-center gap-3"><div className="flex size-14 items-center justify-center rounded-xl bg-[#e8f7fa] text-[#4f8ca3]"><CircleCheck size={26} /></div><div><h2 className="font-black text-[#173d4e]">{currentItem.name}</h2><p className="text-sm font-semibold text-[#477d8d]">{currentItem.quantity} {currentItem.unit} available</p><p className="mt-1 inline-flex items-center gap-1 rounded-full bg-[#fff8d8] px-2 py-0.5 text-[10px] font-bold text-[#9b7800]"><Clock3 size={11} /> Expires {currentItem.expires.toLowerCase()}</p></div></div></div><h2 className="text-xl font-black leading-6 text-[#173d4e]">{choice === 'some' ? `How much did you use?` : `Did you use all ${currentItem.quantity} ${currentItem.unit} of ${currentItem.name}, or do you have leftovers?`}</h2>{choice !== 'some' ? <div className="space-y-2"><button type="button" onClick={chooseAll} disabled={isSaving} className="flex w-full items-center gap-3 rounded-2xl border border-[#cde6ed] bg-[#f0fafb] p-3 text-left hover:bg-white"><span className="flex size-9 items-center justify-center rounded-full bg-[#d9f5f8] text-[#145d72]"><Check size={18} /></span><span><strong className="block text-sm text-[#173d4e]">Used all</strong><small className="text-xs text-[#6f8b95]">Mark {currentItem.quantity} {currentItem.unit} as consumed</small></span></button><button type="button" onClick={chooseSome} disabled={isSaving} className="flex w-full items-center gap-3 rounded-2xl border border-[#cde6ed] bg-[#f0fafb] p-3 text-left hover:bg-white"><span className="flex size-9 items-center justify-center rounded-full bg-[#d9f5f8] text-[#20bed0]"><Clock3 size={18} /></span><span><strong className="block text-sm text-[#173d4e]">Used some</strong><small className="text-xs text-[#6f8b95]">I have some left over</small></span></button><button type="button" onClick={chooseNone} disabled={isSaving} className="flex w-full items-center gap-3 rounded-2xl border border-[#cde6ed] bg-[#f0fafb] p-3 text-left hover:bg-white"><span className="flex size-9 items-center justify-center rounded-full bg-[#e5f0f2] text-[#477d8d]"><X size={18} /></span><span><strong className="block text-sm text-[#173d4e]">Didn&apos;t use it</strong><small className="text-xs text-[#6f8b95]">Leave it in inventory as is</small></span></button></div> : <div className="space-y-3"><label className="flex h-14 items-center rounded-2xl border border-[#b8d7de] bg-white px-3"><input autoFocus type="number" min={step} max={currentItem.quantity} step={wholeUnits.has(currentItem.unit) ? 1 : 0.01} value={someAmount} onChange={(event) => setSomeAmount(event.target.value)} className="min-w-0 flex-1 bg-transparent text-xl font-bold text-[#173d4e] outline-none" /><span className="font-black text-[#173d4e]">{currentItem.unit}</span></label><div className="flex gap-2"><button type="button" onClick={() => setSomeAmount(String(Math.max(step, Number((currentItem.quantity * 0.25).toFixed(2)))))} className="rounded-full border border-[#b8d7de] px-3 py-1 text-xs font-bold text-[#145d72]">25%</button><button type="button" onClick={() => setSomeAmount(String(Math.max(step, Number((currentItem.quantity / 2).toFixed(2)))))} className="rounded-full border border-[#b8d7de] px-3 py-1 text-xs font-bold text-[#145d72]">Half</button><button type="button" onClick={() => setSomeAmount(String(currentItem.quantity))} className="rounded-full border border-[#b8d7de] px-3 py-1 text-xs font-bold text-[#145d72]">All</button></div><div className="grid grid-cols-2 gap-2"><Button type="button" variant="outline" onClick={() => setChoice(null)} className="h-11 border-[#b8d7de] bg-[#f0fafb] text-[#173d4e]">Back</Button><Button type="button" onClick={continueSome} className="h-11 bg-[#20bed0] text-[#063e4d] hover:bg-[#0db3c8]">Continue</Button></div></div>}{saveError && <p role="alert" className="rounded-xl bg-[#fff0ef] px-3 py-2 text-sm text-[#ad4147]">{saveError}</p>}</div></div>
}

...
export function NotificationsPage() {
  ...
  return <div className="w-full max-w-3xl space-y-8">
    ...
    <div>
      ...
      <h1 className="mt-2 text-4xl font-black tracking-tight text-[#193b5a]">Kitchen alerts</h1>
    ...
    <section className="flex flex-col gap-4 rounded-3xl bg-[#eaf9fc] p-5 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <p className="font-bold text-[#193b5a]">Get browser reminders</p>
    ...
    {isLoading ? <div className="rounded-3xl border border-[#bdebf0] bg-white p-10 text-center text-[#6f8b95]">Checking your kitchen...</div> : notifications.length === 0 ? <div className="rounded-3xl border border-dashed border-[#bdebf0] bg-white p-10 text-center"><Bell size={24} className="mx-auto text-[#20c4d6]" /><p className="mt-4 font-bold text-[#193b5a]">You are all caught up.</p><p className="mt-2 text-sm text-[#6f8b95]">New updates will appear here when your kitchen changes.</p></div> : <section className="space-y-3"><div className="flex items-center gap-2 text-sm font-bold uppercase tracking-[0.12em] text-[#6f8b95]"><Clock3 size={16} className="text-[#20c4d6]" /> {notifications.length} notification{notifications.length === 1 ? '' : 's'}</div>{notifications.map((notification) => <Link key={notification.id} to={notification.href} className="flex items-center gap-4 rounded-3xl border border-[#bdebf0] bg-white p-4 shadow-[0_4px_12px_rgba(20,93,114,0.04)] transition-transform hover:-translate-y-0.5"><div className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#eaf9fc] text-[#20c4d6]"><notification.Icon size={20} /></div><div className="min-w-0 flex-1"><h2 className="font-bold text-[#193b5a]">{notification.title}</h2><p className="mt-1 text-sm text-[#6f8b95]">{notification.body}</p>{'item' in notification && notification.item && <div className="mt-2"><FreshnessBadge state={notification.item.freshness} /></div>}</div></Link>)}</section>}
  </div>
}

...
export function RescuePage() {
  ...
  return <div className="-mx-5 -my-8 min-h-full w-[calc(100%+2.5rem)] space-y-6 bg-[#eaf8fa] px-5 py-6 pb-8 sm:-mx-8 sm:-my-12 sm:w-[calc(100%+4rem)] sm:px-8 sm:py-8"><header><p className="text-xs font-bold text-[#193b5a]">Food rescue</p><h1 className="mt-1 text-[2rem] font-black leading-none text-[#193b5a]">Rescue my food</h1><p className="mt-3 text-sm font-semibold text-[#6f8b95]">{urgent.length} items need attention · help prevent waste</p></header>{loading ? <div className="rounded-3xl border border-[#c6dde5] bg-white p-10 text-center text-[#6f8b95]">Checking your kitchen...</div> : <>{urgent.length + expired.length > 0 && <AiRescuePlan items={[...urgent, ...expired]} />}{recommendations.length > 0 && <section className="space-y-3"><div className="flex items-center justify-between px-1"><div><p className="text-xs font-black uppercase tracking-[0.1em] text-[#193b5a]">Cook before it goes to waste</p><h2 className="mt-1 text-lg font-black text-[#193b5a]">Dinner ideas</h2></div><Link to="/app/recipes" className="text-xs font-black text-[#087c91]">See all</Link></div><div className="-mx-1 flex snap-x gap-3 overflow-x-auto px-1 pb-2">{recommendations.map((recipe) => <div key={recipe.id} className="w-[18rem] shrink-0 snap-start"><RecipeCard recipe={recipe} compact /></div>)}</div></section>}{group('rescue-today', rescueToday)}{group('use-soon', useSoon)}{group('expired', expired)}{urgent.length === 0 && expired.length === 0 && <div className="rounded-3xl border border-dashed border-[#b8d4df] bg-white p-10 text-center text-[#6f8b95]">Nothing needs rescuing yet.</div>}</>}</div>
}

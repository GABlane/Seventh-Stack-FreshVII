...
export function FoodCard({ item }: { item: FoodItem }) {
  return (
    <Link to={`/food/${item.id}`} className="group flex min-h-36 flex-col justify-between rounded-3xl border border-[#e5e1d5] bg-white p-4 shadow-[0_8px_24px_rgba(70,67,52,0.05)] transition-transform hover:-translate-y-1">
      ...
      <div className="mt-4 flex items-end justify-between gap-3">
        <div><h3 className="font-bold text-stone-900">{item.name}</h3><p className="mt-1 text-sm text-stone-500">{item.quantity} {item.unit} · {item.expires}</p></div>
